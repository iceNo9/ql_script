...
职责：
    - 确保数据库中存在账号行。
    - 所有对 Account 实体字段的写入（金币 / 签到数据 / 状态标记）。
...
class AccountService:
    ...
    def ensure(self, account: HifitiAccountConfig) -> Account:
        """
        获取数据库账号，不存在则创建。
    ...
    def update_gold(
        ...
        """同步账号金币（用户数据查询用）。"""
        db_account.gold = gold
        self.account_repository.update(db_account)

        logger.debug(
            "账号金币更新: username=%s, gold=%d",
            db_account.username,
            gold,
        )
    ...
    def apply_remote_checkin(
        ...
        *,
        ...
        logger.info(
            "账号远程签到同步写入: username=%s, last_checkin_at=%s",
            db_account.username,
            checkin_utc,
        )
