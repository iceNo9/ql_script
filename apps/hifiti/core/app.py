...
认证降级策略：
    任何 HifitiAPIError 都视为"可能需要重新认证"，
    触发认证来源降级（最多 3 次）。
    不再区分 HTTP 内部错误码。
"""
...
from apps.hifiti.core.account import AccountService
from apps.hifiti.core.api import HifitiAPI, HifitiAPIError
from apps.hifiti.core.auth import AuthService, AuthSource
from apps.hifiti.core.checkin import CheckinOutcome, CheckinService
...
from apps.hifiti.core.report import ReportService
...
class HifitiApp:
    ...
    def _authenticate_and_run(
        ...
        """
        认证并执行 action，遇到 HifitiAPIError 时降级重试。
        ...
        raise last_error or HifitiAPIError(
            status_code=0,
    ...
    def _run_for_account(
        self,
        username: str,
        action: Callable[[HifitiAccountConfig, Account], T],
    ) -> T | None:
        """
        单账号执行模板。
    ...
    def _run_for_all(
        ...
        if not self.hifiti_config.accounts:
            logger.warning("没有配置 Hifiti 账号，跳过执行")
            return results
    ...
    def checkin(self, username: str) -> HifitiCheckinResult | None:
        """
        指定账号执行签到。
        ...
        if outcome.should_update_account:
            if outcome.action == "checked_in":
                self.account_service.apply_checkin_success(
                    db_account,
                    checkin_gold=outcome.checkin_gold,
    ...
    def get_user_data(
        ...
        def action(
            ...
            if result is not None:
                self.account_service.update_gold(db_account, result.gold)
            return result
    ...
    def get_user_data_all(
        self,
    ) -> dict[str, HifitiUserDataResult | None]:
        """获取所有账号的用户数据。"""
    ...
    def _fetch_user_data(
        ...
        注意：
            本方法不吞 HifitiAPIError。
            若 cookie 失效，异常向上抛给 _authenticate_and_run 处理降级。
            仅"解析失败"（result.success=False）返回 None。
        """
        logger.info("获取 Hifiti 用户数据: username=%s", account.username)
    ...
    def build_report_html(self) -> str:
        """构建 Hifiti HTML 运行报告。"""
        return self.report_service.build_html(self.hifiti_config.accounts)
