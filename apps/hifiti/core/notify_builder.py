# apps/hifiti/core/notify_builder.py
...
class SectionBuilder:
    ...
    def account_section(
        ...
        total = len(accounts)
        avg_gold = sum(account.gold for account in accounts) / total if total > 0 else 0
        avg_continuous = (
        ...
        ">
            ...
            ">
                平均金币:
                <strong style="color:{cls.COLORS["gold"]};">{avg_gold:.1f}</strong>
                |
                平均连续签到:
    ...
    def checkin_section(
            ...
            rows.append(
                cls._row(
                    [
                        ...
                        cls._cell(
                            f"+{result.checkin_gold}",
                            color=cls.COLORS["gold"],
                            bold=True,
                        ),
                        cls._cell(
                            (
                                f"#{result.checkin_rank}"
                                if result.checkin_rank > 0
                                else "-"
                            ),
                            color=cls.COLORS["info"],
                            bold=True,
        ...
        total_gold = sum(result.checkin_gold for result in results if result.success)
        ...
        ">
            ...
            ">
                ...
                |
                获得金币:
                <strong style="color:{cls.COLORS["gold"]};">+{total_gold}</strong>
                |
                成功率:
