...
职责：
    - 判断周常任务是否达到执行间隔
    - 执行周常任务（申请 + 完成）
    - 产出"周常任务决策结果"（WeeklyTaskOutcome）
    - 写入周常任务完成日志（WeeklyCompleteLog）

不负责：
    - 认证（门面负责，cookie 由门面提前设好）
    - Account 字段写入（AccountService 负责）
    - 事务提交（门面负责）
    - 日常任务（DailyTaskService 负责）
...
from apps.southpro.core.notify_dto import WeeklyTaskInfo
from apps.southpro.core.parser import (
    SouthProParser,
    SouthProWeeklyCompleteResult,
)
from apps.southpro.core.repositories import WeeklyCompleteLogRepository
from utils.log import get_logger
from utils.paths import logs
from utils.timezone import now_local, now_utc, utc_to_local

logger = get_logger(
    name="southpro_weekly",
    log_dir=logs(),
    fmt_type="detailed",
)
...
class WeeklyTaskOutcome:
    ...
    字段说明：
        result: 给调用方的原始 SouthProWeeklyCompleteResult。
        action: 本次动作类型，取值：
            - "skipped"     : 未达到执行间隔，跳过
    ...
    result: SouthProWeeklyCompleteResult
    action: str
...
class WeeklyTaskService:
    ...
    持有 api / parser / weekly_complete_log_repository，
    自行发起远程调用、写周常日志；
    不写 Account 字段，不提交事务。
    """
    ...
    def __init__(
        ...
        weekly_complete_log_repository: WeeklyCompleteLogRepository,
    ) -> None:
        self.api = api
        self.parser = parser
        self.weekly_complete_log_repository = weekly_complete_log_repository
    ...
    def is_due(self, db_account: Account) -> bool:
        """判断当前账号是否达到周常任务执行间隔。"""
        if db_account.last_weekly_complete_at is None:
            return True

        last_local = utc_to_local(db_account.last_weekly_complete_at)
    ...
    def next_complete_at(self, db_account: Account) -> datetime | None:
        ...
        last_local = utc_to_local(db_account.last_weekly_complete_at)
        return last_local + self.WEEKLY_INTERVAL
    ...
    def build_report_infos(
        ...
        *,
        ...
        """
        构造 WeeklyTaskInfo 列表（供 ReportService 使用）。
        ...
        for config in account_configs:
            ...
            last_local = (
                utc_to_local(db_account.last_weekly_complete_at)
                if db_account.last_weekly_complete_at is not None
                else None
            )

            next_local = (
                last_local + self.WEEKLY_INTERVAL if last_local is not None else None
            )
    ...
    def _do_complete(
        ...
        try:
            ...
            apply_result = self.parser.parse_apply_weekly(response)
            ...
            if not apply_result.success:
                ...
                return self._build_failure_outcome(
                    db_account,
                    error,
                    SouthProWeeklyCompleteResult.failure(error),
                )
            ...
            complete_result = self.parser.parse_complete_weekly(response)
            ...
            logger.info(
                "SouthPro 周常任务完成: username=%s, delta_points_sp=%d",
                account.username,
            ...
            logger.warning(
                "周常任务 API 异常，上抛给门面处理降级: username=%s",
                account.username,
            )
            raise
            ...
            return self._build_failure_outcome(
                db_account,
                str(exc) or "周常任务异常",
                SouthProWeeklyCompleteResult.failure("周常任务异常"),
            )
