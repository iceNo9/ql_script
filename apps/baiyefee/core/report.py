...
职责：
    从数据库读取账号 / 签到日志，构建 Baiyefee HTML 运行报告。
...
class ReportService:
    ...
    只读服务：仅依赖两个 Repository，不持有 Session，
    不提交事务。
    """
    ...
    def build_html(
        self,
        account_configs: list[BaiyefeeAccountConfig],
    ) -> str:
        """
