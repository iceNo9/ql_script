        ...
        try:
            ...
            if after_date := search_criteria.get("after_date"):
                if isinstance(after_date, datetime.datetime):
                    criteria_parts.append(f'SINCE {after_date.strftime("%d-%b-%Y")}')
                elif isinstance(after_date, str):
                    criteria_parts.append(f'SINCE {after_date}')
            ...
            for msg_id in reversed(ids):
                try:
                    ...
                    logger.error(f"[!] 查询邮件 {msg_id} 失败: {e}")
                    continue
