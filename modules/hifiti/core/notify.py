...
class Notifier:
    def __init__(
        ...
        self.report_id = f"HFT{datetime.now().strftime('%Y%m%d%H%M%S')}{random.randint(1000, 9999)}"

    def _build_account_rows(self) -> str:
        """构建账户信息表格行"""
        if not self.account_infos:
            return '<tr><td colspan="2" style="border:1px solid #e0e0e0; padding:20px; text-align:center; color:#999;">暂无数据</td></tr>'
        ...
        for acc in self.account_infos:
            ...
            <tr>
                ...
                <td style="border:1px solid #e0e0e0; padding:10px 12px; text-align:center; color:#ffc107; font-weight:bold;">{acc.coins}</td>
            </tr>
            ''')
        return ''.join(rows)
    ...
    def _build_sign_rows(self) -> str:
        ...
        for res in self.sign_results:
            ...
            <tr>
                ...
                <td style="border:1px solid #e0e0e0; padding:10px 12px; text-align:center;">{res.rank}</td>
                ...
                <table style="width:100%; border-collapse:collapse; font-size:13px; min-width:300px;">
                    <thead>
                        <tr style="background:#f0f0f0;">
                            ...
                            <th style="border:1px solid #e0e0e0; padding:10px 12px; text-align:center;">金币</th>
                        </tr>
                    </thead>
                    <tbody>
    ...
    def _build_sign_section(self) -> str:
        ...
        <div style="background:#ffffff; border:2px solid #4CAF50; border-radius:8px; margin-bottom:25px; overflow:hidden;">
            ...
            <div style="padding:10px 20px; background:#f8f9fa; border-bottom:1px solid #e0e0e0; font-size:12px;">
                ...
                获得金币: <strong style="color:#ffc107;">+{total_reward}</strong> | 
                ...
                <table style="width:100%; border-collapse:collapse; font-size:13px; min-width:500px;">
                    <thead>
                        <tr style="background:#e8f5e9;">
                            ...
                            <th style="border:1px solid #e0e0e0; padding:10px 12px; text-align:center;">获得金币</th>
                            <th style="border:1px solid #e0e0e0; padding:10px 12px; text-align:center;">排名</th>
    ...
    def send(self, subject: str = "🎯 HiFiTi 签到报告") -> bool:
        """发送运行报告邮件"""
