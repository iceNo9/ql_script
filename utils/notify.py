...
def smtp_origen(title: str, content: str) -> None:
    """
    使用 SMTP 邮件 推送消息。
    """
    if (
...
def smtp(title: str, content: str) -> None:
    ...
    SMTP_HTML=true  时发送 HTML 邮件
    SMTP_HTML=false 时发送纯文本邮件
    """
    if (
        not push_config.get("SMTP_SERVER")
        or not push_config.get("SMTP_SSL")
    ...
    # 根据 SMTP_HTML 决定邮件正文类型
    html = push_config.get("SMTP_HTML") == "true"
    mime_subtype = "html" if html else "plain"

    message = MIMEText(content, mime_subtype, "utf-8")

    message["From"] = formataddr(
        (
            Header(
                push_config.get("SMTP_NAME"),
                "utf-8",
            ).encode(),
            push_config.get("SMTP_EMAIL"),
        )
...
def ntfy(title: str, content: str) -> None:
    ...
    if push_config.get("NTFY_TOKEN"):
        ...
        authStr = (
            push_config.get("NTFY_USERNAME") + ":" + push_config.get("NTFY_PASSWORD")
        )
